#!/usr/bin/env python3
"""
Optimize ethnicity-specific character images for EP-034.
...
# Configuration
BASE_DIR = Path(__file__).parent.parent.parent / "apps/web/public/images/ethnicity"
...
def main():
    ...
    # Find all ethnicity images (in female/ and male/ subdirectories)
    image_files = []
    for gender_dir in ['female', 'male']:
        gender_path = BASE_DIR / gender_dir
        if gender_path.exists():
            image_files.extend(list(gender_path.glob("*-ethnicity*.webp")))
    
    if not image_files:
        print(f"No ethnicity images found in {BASE_DIR}", file=sys.stderr)
        sys.exit(1)
    
    print(f"Found {len(image_files)} ethnicity images to optimize")
    ...
    for img_path in sorted(image_files):
        ...
        if original_size > 0:
            ...
            print(f"{status} {img_path.name:30} {original_size/1024:7.1f} KB → {compressed_size/1024:7.1f} KB "

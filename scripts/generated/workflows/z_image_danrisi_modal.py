...
app = modal.App(name="ryla-z_image_danrisi", image=image)
...
class Z_image_danrisi:
    """ComfyUI server for z_image_danrisi workflow."""
    ...
    def generate(self, workflow_json: dict, **params):
        """
        Execute z_image_danrisi workflow.
    ...
    def fastapi_app(self):
        ...
        fastapi = FastAPI(title="ComfyUI z_image_danrisi API")
        ...
        @fastapi.get("/")
        async def root():
            return {"status": "ok", "app": "z_image_danrisi"}

        @fastapi.get("/health")
        async def health():
            return {"status": "healthy", "app": "z_image_danrisi"}

...
export function InfluencerScarsStep() {
    const { nextStep } = useStepperContext();
    const form = useFormContext<FunnelSchema>();

    const scars = form.watch("influencer_scars");

    const options = INFLUENCER_SKIN_FEATURES.scars.map((option): ImageOption => ({
        id: option.id,
        value: option.value,
        image: {
            ...option.image,
            src: getSkinFeatureImage("scars", option.value) || option.image.src,
        },
    }));
    ...
    const handleScarsSelect = (value: string) => {
        ...
        safePostHogCapture('funnel_option_selected', {
            step_name: 'Scars',
            step_index: 14,
            option_type: 'influencer_scars',
            option_value: value,
        });
    };

    const handleScarsSelectWithAutoAdvance = useAutoAdvanceOnSingleOption(
        options,
        handleScarsSelect
    );
            ...
            <div className="max-w-[450px] flex-1 h-full w-full mx-auto flex flex-col items-center justify-center">
                <div className="w-full flex-1 sm:flex-none flex flex-col items-center sm:justify-center mb-[30px] sm:mb-[70px]">
                    ...
                    <p className="text-white/90 text-base font-medium mb-8 px-4 text-center">
                        Select scar presence for your AI influencer
                    </p>
                    ...
                    <ImageOptionsGrid
                        ...
                        renderOption={(option) => (
                            <ImageCard
                                ...
                                onClick={() => handleScarsSelectWithAutoAdvance(option.value)}
                            >
                                <ImageCard.Image />
                                <ImageCard.Name />
                                <ImageCard.Overlay />

...
import { X, Loader2Icon, Clock, Shield } from "lucide-react";
...
export function PaymentFormStep() {
    ...
    const { product, onSubmit, isPending, finbyIframeRef } = usePaymentForm(posthog);
    ...
    const countdown = useCountdown({ hours: 0, minutes: 37, seconds: 34 }, { autoStart: true });
    ...
    useEffect(() => {
        ...
        try {
            const previousStepData = localStorage.getItem("finby_payment_previous_step");
            if (previousStepData) {
                ...
                if (
                    ...
                    console.log(`Restoring to previous step from PaymentFormStep: ${step}`);
                    setStep(step);
        ...
        if (paymentResult) {
            try {
                ...
                if (result.resultCode === 0 || result.resultCode === 3) {
                    const allSpotsReservedIndex = getStepIndexByName("All Spots Reserved");
            ...
            <div className="max-w-[450px] flex-1 h-full w-full mx-auto flex flex-col items-center justify-center">
                <div className="w-full flex-1 sm:flex-none flex flex-col items-center sm:justify-center">
                    ...
                    <div className="w-full mb-6">
                        <Label className="text-white font-bold text-lg sm:text-xl mb-4 block" htmlFor="email">
                            ...
                            render={({ field }) => (
                                <div className="border-2 border-purple-400/50 rounded-lg bg-gradient-to-br from-purple-500/10 to-pink-500/10 p-1 shadow-lg">
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (emailValue.length === 1 && !emailTrackedRef.current.has('started')) {
                                        ...
                                        if (emailValue && emailRegex.test(emailValue) && !emailTrackedRef.current.has(emailValue)) {
                                            ...
                                            updateSessionEmail(sessionId, emailValue).catch((error) => {
                                                console.error("Failed to save email to session:", error);
                                            });
                        ...
                        />
                        {emailError && (
                            <p className="text-red-400 text-xs mt-2 font-medium">{emailError.message}</p>
                        )}
                    </div>
                    ...
                    <div className="w-full bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-4 shadow-xl mb-6">
                        ...
                        <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-2">
                                ...
                                <p className="text-white/90 text-xs font-medium">SSL Encrypted</p>
                            </div>
                            <div className="flex items-center gap-2">
                                ...
                                <p className="text-white/90 text-xs font-medium">Cancel Anytime</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="w-full bg-gradient-to-br from-green-500/10 to-emerald-500/5 backdrop-blur-sm border border-green-400/30 rounded-xl p-4 shadow-xl mb-6">
                        <div className="flex items-center justify-between mb-3">
                            <h3 className="text-white font-bold text-base">30-Day Money-Back Guarantee</h3>
                    ...
                    <div className="w-full flex flex-col items-center gap-3">
                        ...
                        <div className="flex items-center justify-center gap-4 pt-2">
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-white/50 text-xs font-medium">Secure</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                                <svg
                                ...
                                <span className="text-white/50 text-xs font-medium">Encrypted</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                                <svg
                                ...
                                <span className="text-white/50 text-xs font-medium">Protected</span>
                            </div>
                        </div>
                    </div>
                </div>

...
export function AccessInfluencerStep() {
  ...
  const _enable_nsfw = form.watch('enable_nsfw');
  const _video_content_options = form.watch('video_content_options') || [];
  const _enable_lipsync = form.watch('enable_lipsync');
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      if (cr)
        setPreviewSize({ w: Math.round(cr.width), h: Math.round(cr.height) });
    });
    ro.observe(el);
    ...
    switch (label) {
      ...
      case 'Age': {
        ...
        if (ageNum >= 18 && ageNum <= 25)
          return getAgeRangeImage(ethnicity, '18-25');
        if (ageNum >= 26 && ageNum <= 33)
          return getAgeRangeImage(ethnicity, '26-33');
        if (ageNum >= 34 && ageNum <= 41)
          return getAgeRangeImage(ethnicity, '34-41');
        if (ageNum >= 42 && ageNum <= 50)
          return getAgeRangeImage(ethnicity, '42-50');
        return null;
      }
      case 'Skin Color':
        if (!ethnicity) return null;
      ...
      case 'Body Type': {
        ...
        const bodyType = INFLUENCER_BODY_TYPES.find(
          (type) => type.value === bodyTypeValue
        );
      ...
      case 'Ass Size': {
        ...
        const assSize = INFLUENCER_ASS_SIZES.find(
          (size) => size.value === assSizeValue
        );
  ...
  const traits = [
    ethnicity && {
      label: 'Ethnicity',
      value: formatLabel(ethnicity),
      rawValue: ethnicity,
    },
    age && {
      label: 'Age',
      value: formatLabel(String(age)),
      rawValue: String(age),
    },
    skin_color && {
      label: 'Skin Color',
      value: formatLabel(skin_color),
      rawValue: skin_color,
    },
    eye_color && {
      label: 'Eye Color',
      value: formatLabel(eye_color),
      rawValue: eye_color,
    },
    hair_style && {
      label: 'Hair Style',
      value: formatLabel(hair_style),
      rawValue: hair_style,
    },
    hair_color && {
      label: 'Hair Color',
      value: formatLabel(hair_color),
      rawValue: hair_color,
    },
    face_shape && {
      label: 'Face Shape',
      value: formatLabel(face_shape),
      rawValue: face_shape,
    },
    body_type && {
      label: 'Body Type',
      value: formatLabel(body_type),
      rawValue: body_type,
    },
    ass_size && {
      label: 'Ass Size',
      value: formatLabel(ass_size),
      rawValue: ass_size,
    },
    breast_type && {
      label: 'Breast Type',
    ...
    tattoos && {
      label: 'Tattoos',
      value: formatLabel(tattoos),
      rawValue: tattoos,
    },
    piercings && {
      label: 'Piercings',
      value: formatLabel(piercings),
      rawValue: piercings,
    },
    freckles && {
      label: 'Freckles',
      value: formatLabel(freckles),
      rawValue: freckles,
    },
    ...
    .filter(
      (trait): trait is { label: string; value: string; rawValue: string } =>
        Boolean(trait)
    )
    .map((trait) => ({
      ...trait,

...
export const defaultValues = {
    ...
    // Payment
    productId: subscriptions.find((subscription) => subscription.isBestChoice)?.productId || null,
...
const NSFW_CONTENT_STEP_INDEX = FUNNEL_STEPS.findIndex((step) => step.name === "NSFW Content");
const LIPSYNC_STEP_INDEX = FUNNEL_STEPS.findIndex((step) => step.name === "Lipsync Feature");
...
export function useFunnelForm() {
    ...
    const nextStep = () => {
        ...
        console.log(
            `nextStep called for step ${active} (${currentStepName}), type: ${currentStepType}, formField: ${JSON.stringify(fieldsToValidate)}`,
        );
        ...
        if (
            ...
            console.log(`Skipping validation for ${currentStepType} step: ${currentStepName}`);
            ...
            setActive((current) => {
                ...
                if (next >= STEPS_COUNT) {
                    console.warn(
                        `Attempted to go beyond step ${STEPS_COUNT - 1}, staying at step ${current}`,
                    );
                    setIsNavigating(false); // Reset if can't advance
                    return current;
                }
                console.log(`Step progression (info step, no validation): ${current} -> ${next}`);
                ...
                .then((result) => {
                    ...
                    if (result) {
                        ...
                        safePostHogCapture("funnel_step_validation_failed", {
                            ...
                            trigger_field: Array.isArray(validationFields) ? validationFields.join(",") : validationFields[0] || "",
                        });

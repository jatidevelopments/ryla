...
import { getPaymentStepIndices, getStepIndexByName } from "@/features/funnel/config/steps";
...
export default function FunnelView() {
    ...
    useEffect(() => {
        ...
        const currentStep = orderedSteps.find((step) => step.index === stepper.value);
    ...
    useEffect(() => {
        ...
        const urlChanged = stepFromUrl !== null && stepFromUrl !== lastStepFromUrl.current;
        ...
        if (isInitialMount.current) {
            ...
            if (stepFromUrl !== null) {
                // URL has step - use it
                const authState = readPersistedState<{ authToken: string | null }>("auth-storage");
                ...
                const hasPaymentError = typeof window !== "undefined" && localStorage.getItem("finby_payment_error");
                const hasPaymentResult = typeof window !== "undefined" && localStorage.getItem("finby_payment_result");
                const isFromPaymentCallback = typeof window !== "undefined" && sessionStorage.getItem("finby_payment_callback_redirect") === "true";
                ...
                const isPaymentErrorStep = stepFromUrl === 34 && (hasPaymentError || hasPaymentResult || isFromPaymentCallback);
                ...
                if (isPaymentStep && (!hasValidAuth || !hasFunnel) && !isPaymentErrorStep) {
                    ...
                    console.log("Redirecting from payment step - no auth/funnel and no payment error", {
                        stepFromUrl,
                        hasValidAuth,
                        hasFunnel,
                        hasPaymentError,
                    ...
                    current.delete("step");
                    if (pathname) {
                        router.replace(current.size ? `${pathname}?${current.toString()}` : pathname);
                    }
                    stepper.onChange(0);
                    setStepInStore(0);
                ...
                const initialStep = savedStep !== null && savedStep !== undefined ? savedStep : 0;
        ...
        if (urlChanged && stepFromUrl !== null) {
            ...
            if (isPaymentStep && (!hasValidAuth || !hasFunnel) && !isPaymentErrorStep) {
                // Redirect to start if trying to access payment steps without auth
                // UNLESS it's a payment error step (step 34 with error message or result)
                console.log("Redirecting from payment step (URL changed) - no auth/funnel and no payment error", {
                    stepFromUrl,
                    hasValidAuth,
                    hasFunnel,
                    hasPaymentError,
    ...
    }, [isReady, searchParams, stepper.value, router, pathname, setStepInStore, orderedSteps.length]);

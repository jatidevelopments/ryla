...
import {
  OutfitModeSelector,
  type OutfitMode,
} from '../../../../components/studio/generation/pickers/OutfitModeSelector';
...
function StudioContent() {
  ...
  const [showOutfitModeSelector, setShowOutfitModeSelector] =
  ...
  React.useEffect(() => {
    refreshExistingImages().catch((err) =>
  ...
  const handleTemplateApply = React.useCallback(
    async (templateId: string) => {
      try {
        const { config } = await utils.templates.applyTemplate.fetch({
          id: templateId,
        });
  ...
  const handleGenerate = async () => {
    ...
    if (!influencer?.avatar) {
      throw new Error(
        'Missing base image for this influencer. Please re-run the wizard base image step.'
      );
    }
      ...
      while (attempts < 120) {
        ...
        if (res.status === 'completed' && res.images?.[0]?.url) {
          ...
          const newPost: Post = {
            ...
            environment: settings.environment,
            outfit:
              typeof settings.outfit === 'string'
                ? settings.outfit
                : influencer?.outfit || 'casual',
            aspectRatio: settings.aspectRatio,
    ...
    <div className="flex flex-col min-h-screen">
                  ...
                  {existingImages.length > 0 && (
                    ...
                    >
                      <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] text-[var(--text-muted)]">
                                  {selectedExistingPost.scene} •{' '}
                                  {selectedExistingPost.environment}
                                </p>
                              </div>
                            </div>
                        ...
                        <p className="text-[10px] text-[var(--text-muted)]">
                          Select an image to copy its scene, environment &
                          outfit settings
                        </p>
                      </div>
                    </SettingsSection>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {SCENE_OPTIONS.map((scene) => (
                        <OptionChip
                          ...
                          onSelect={() =>
                            handleSettingChange('scene', scene.value)
                          }
                        />
                      ))}
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {ENVIRONMENT_OPTIONS.map((env) => (
                        <OptionChip
                          ...
                          onSelect={() =>
                            handleSettingChange('environment', env.value)
                          }
                        />
                      ))}
                    </div>
                        ...
                        <div className="flex-1 rounded-lg border border-[var(--border-default)] bg-[var(--bg-surface)] px-3 py-2">
                          <div className="flex items-center justify-between">
                            ...
                            {settings.outfit && (
                              <button
                                onClick={() =>
                                  handleSettingChange('outfit', null)
                                }
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <select
                          ...
                          value={
                            settings.modelProvider === 'comfyui'
                              ? 'comfyui'
                              : settings.modelId
                          }
                          onChange={(e) => {
      ...
      {generatedPost && (
        <SaveTemplateDialog
          ...
          defaultName={`${
            SCENE_OPTIONS.find((s) => s.value === settings.scene)?.label ||
            settings.scene
          } - ${
            ENVIRONMENT_OPTIONS.find((e) => e.value === settings.environment)
        ...
        <div className="mx-auto max-w-7xl px-4 lg:px-6">
          <div className="flex items-center justify-between gap-4 py-3">
            ...
            <div className="flex items-center gap-2 overflow-x-auto scroll-hidden">
              ...
              <button
                onClick={() => {
                  const sceneSection = document.querySelector(
                    '[data-section="scene"]'
                  );
                  sceneSection?.scrollIntoView({
                    behavior: 'smooth',
                    block: 'center',
                  });
                }}
              ...
              >
                ...
                <span>
                  {SCENE_OPTIONS.find((s) => s.value === settings.scene)
                    ?.label || 'Scene'}
                </span>
              </button>
              ...
              >
                ...
                <span>
                  {ENVIRONMENT_OPTIONS.find(
                    (e) => e.value === settings.environment
                  )?.label || 'Environment'}
                </span>
              </button>

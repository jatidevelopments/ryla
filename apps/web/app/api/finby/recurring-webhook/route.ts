...
const v3Handler = createFinbyV3WebhookHandler({
  ...
  onPaymentSucceeded: async (event) => {
    console.log(
      '[Finby Recurring Webhook] Payment succeeded (v3):',
    ...
    const originalPaymentRequestId =
      rawData?.data?.originalPaymentRequestId ||
    ...
    if (!originalPaymentRequestId) {
      console.warn(
        '[Finby Recurring Webhook] No OriginalPaymentRequestId found'
      );
      return;
    }

    // Find subscription by original payment request ID
    const subscription = await paymentRepo.getSubscriptionByFinbyId(
      originalPaymentRequestId
    );
    if (!subscription) {
      console.warn(
        `[Finby Recurring Webhook] Subscription not found for OriginalPaymentRequestId: ${originalPaymentRequestId}`
      );
      return;
    }
    ...
    if (subscription.cancelAtPeriodEnd || subscription.status === 'cancelled') {
      console.log(
        `[Finby Recurring Webhook] Subscription ${subscription.id} is cancelled, skipping renewal`
      );
      return;
    }
    ...
    console.log(
      `[Finby Recurring Webhook] Updated subscription ${
        subscription.id
      } period to ${currentPeriodEnd.toISOString()}`
    );
    ...
    if (subscription.tier) {
      ...
      if (credits > 0) {
        ...
        console.log(
          `[Finby Recurring Webhook] Granted ${credits} credits for subscription renewal`
        );
      }
    }
  },
  onPaymentFailed: async (event) => {
    console.error(
      '[Finby Recurring Webhook] Payment failed (v3):',
      event.data.error
    );
    ...
    if (originalPaymentRequestId) {
      ...
      if (subscription) {
        ...
        console.log(
          `[Finby Recurring Webhook] Updated subscription ${subscription.id} to past_due`
        );
      }
    }
  },
...
const v1Handler =
  process.env.FINBY_API_KEY && process.env.FINBY_MERCHANT_ID
    ? createFinbyWebhookHandler({
        ...
        onPaymentSucceeded: async (event) => {
          console.log(
            '[Finby Recurring Webhook] Payment succeeded (v1):',
            event.data.invoiceId
          );
          ...
          const originalPaymentRequestId =
            rawData?.data?.originalPaymentRequestId ||
            rawData?.data?.PaymentInformation?.References
              ?.OriginalPaymentRequestId;
          ...
          const currentPeriodEnd =
            event.data.currentPeriodEnd || new Date(currentPeriodStart);
          currentPeriodEnd.setMonth(currentPeriodEnd.getMonth() + 1);
        ...
        onPaymentFailed: async (event) => {
          console.error(
            '[Finby Recurring Webhook] Payment failed (v1):',
            event.data.error
          );

  ...
  if (!selectedImage) {
    return (
      <Tooltip content="Upload image as prompt or for editing">
        <button className="flex items-center justify-center h-11 w-11 rounded-2xl bg-white/5 text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-white/10 transition-all">
          <svg
  ...
  return (
    <div className="flex flex-row items-center gap-2 flex-nowrap">
      ...
      <div className="relative group flex items-center justify-center h-11">
        <div className="relative h-11 w-11 rounded-2xl overflow-hidden border-2 border-[var(--purple-500)] ring-1 ring-[var(--purple-500)]/50 flex-shrink-0">
          <Image
      ...
      {mode === 'editing' && (
        <div className="flex flex-row items-center gap-1.5 flex-nowrap">
          {selectedObjects.slice(0, maxObjects).map((obj, index) => (
            <div
              key={obj.id}
              className="relative group flex items-center justify-center h-11 flex-shrink-0"
            >
              <div className="relative h-11 w-11 rounded-2xl overflow-hidden border-2 border-[var(--purple-400)]/50 flex-shrink-0">
                <Image
          ...
          {selectedObjects.length < maxObjects && (
            <Tooltip content="Add object to edit">
              <button
                onClick={onAddObject}
                className="flex items-center justify-center h-11 w-11 rounded-2xl bg-white/5 text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-white/10 transition-all border-2 border-dashed border-white/20 flex-shrink-0"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 20 20"

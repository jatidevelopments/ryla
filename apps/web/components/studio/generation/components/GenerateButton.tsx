...
export function GenerateButton({
  ...
  // Only disable when can't generate (not when already generating - allow queueing)
  ...
  return (
    <Tooltip content={getTooltipContent()}>
      <button
        ...
        className={cn(
          'h-11 md:h-12 px-5 md:px-8 rounded-full md:rounded-2xl font-bold text-sm flex items-center gap-1.5 md:gap-2.5 transition-all flex-shrink-0 relative',
          isDisabled
      ...
      >
        {/* Show active generation indicator as a small badge, but keep button usable */}
        {isGenerating && (

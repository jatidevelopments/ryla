...
export function SettingsSection({
  ...
  // Get the label for the current quality value
  const qualityLabel = QUALITY_OPTIONS.find(
    (opt) => opt.value === settings.quality
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <Tooltip content="Quality: Choose between Fast (quick), HD (balanced), or Ultra HD (best quality). Higher quality uses more credits.">
          <button
            ref={qualityButtonRef}
            onClick={onToggleQualityPicker}
            className="flex items-center gap-1.5 min-h-[44px] px-3 py-2.5 rounded-2xl bg-white/5 text-[var(--text-primary)] text-sm font-medium hover:bg-white/10 transition-all"
          >
            <svg
      ...
      {showPromptEnhance && (
        <>
          <Tooltip content="Prompt Enhance: Uses AI to improve your prompt, adding more detail and creativity for better generation results.">
            <button
              onClick={() =>
                updateSetting('promptEnhance', !settings.promptEnhance)
              }
              className={cn(
      ...
      <Tooltip content="Batch Size: Generate multiple images at once. Higher batch = more credits used per generation.">
        <div className="flex items-center gap-0.5 h-[44px] px-1.5 rounded-2xl bg-white/5">
          <button
            onClick={() =>
              updateSetting('batchSize', Math.max(1, settings.batchSize - 1))
            }
            disabled={settings.batchSize <= 1}
            className="h-8 w-8 flex items-center justify-center text-[var(--text-muted)] hover:text-[var(--text-primary)] disabled:opacity-30 disabled:cursor-not-allowed transition-colors rounded hover:bg-white/10"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
          ...
          <span className="min-w-[28px] text-center text-sm font-medium text-[var(--text-primary)]">
            {settings.batchSize}/4
          </span>
          <button
            onClick={() =>
              updateSetting('batchSize', Math.min(4, settings.batchSize + 1))
            }

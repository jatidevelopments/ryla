...
const testimonialsRowA: Testimonial[] = [
  {
    quote:
      'Character consistency is absolutely unreal. Same face across 100+ posts.',
    name: 'Sarah M.',
    earnings: '$5.2K/mo',
    platform: 'instagram',
    rating: 5,
  ...
  {
    quote:
      "My followers genuinely can't tell she's AI. The engagement is insane.",
    name: 'Jake T.',
    earnings: '$8.1K/mo',
    platform: 'tiktok',
    rating: 5,
...
const testimonialsRowB: Testimonial[] = [
  ...
  {
    quote:
      'Community support is incredible. Got advice that doubled my income.',
    name: 'Marcus J.',
    earnings: '$9.2K/mo',
    platform: 'tiktok',
    rating: 5,
...
function RichTestimonialCard({ testimonial, className }: TestimonialCardProps) {
  // Safety check: ensure platform exists in config
  const platform =
    testimonial.platform in platformConfig
      ? platformConfig[testimonial.platform]
      : platformConfig.instagram;

  if (!platform) {
    console.error('Invalid platform:', testimonial.platform);
    return null;
  }

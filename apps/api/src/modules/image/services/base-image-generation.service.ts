...
export class BaseImageGenerationService {
  ...
  async generateBaseImages(
    ...
    if (!useModal && !useFal) {
      throw new Error(
        'Neither Modal.com nor FAL available. Set MODAL_WORKSPACE or FAL_KEY in Infisical /apps/api. Base image generation requires Modal.com or FAL (ComfyUI pod/RunPod not supported).'
      );
    }
    ...
    if (input.promptInput) {
      ...
      basePrompt +=
        ', smiling, friendly expression, warm smile, approachable, fully clothed, appropriate attire, professional appearance, modest clothing';
      ...
      if (input.promptEnhance !== false) {
        try {
          ...
          const enhancementPromise = enhancer
            .enhance({
              prompt: basePrompt,
            ...
            .catch((err) => {
              this.logger.warn(
                'Wizard prompt enhancement failed, using base prompt:',
                err
              );
              return null;
            });
          ...
          if (enhancementResult && enhancementResult.enhancedPrompt) {
            ...
            if (
              ...
              !lowerPrompt.includes('hyper-realistic') &&
              !lowerPrompt.includes('ultra-detailed')
            ) {
              basePrompt +=
                ', 8K hyper-realistic, ultra-detailed, professional photography, Shot on Fujifilm GFX 100S, sharp focus, high resolution, photorealistic';
            }
            ...
            if (enhancementResult.negativeAdditions.length > 0) {
              negativePrompt +=
                ', ' + enhancementResult.negativeAdditions.join(', ');
            }
          } else {
          ...
          this.logger.warn(
            'Wizard prompt enhancement failed, using base prompt with quality keywords:',
            err
          );
          // Add quality keywords even when enhancement fails
          basePrompt +=
            ', 8K hyper-realistic, ultra-detailed, professional photography, sharp focus, high resolution, photorealistic';
        }
      } else {
      ...
      if (!input.appearance || !input.identity) {
        throw new Error(
          'Either promptInput or appearance+identity must be provided'
        );
      }
      ...
      const builtPrompt = new PromptBuilder()
        ...
        .addDetails(
          'smiling, warm friendly expression, fully clothed, appropriate attire, professional appearance, modest clothing'
        ) // Add smiling and clothing requirement
        .withNegativePrompt(
    ...
    // Select workflow (use recommended or specified) — kept for future workflow path
    const _workflowId =
    ...
    const _steps = input.steps ?? 9;
    const _cfg = input.cfg ?? 1.0;
    ...
    if (shouldUseModal || shouldUseFal) {
      ...
      if (shouldUseFal) {
        // Use Fal.ai (2 images per model). When Modal unavailable, include Z-Image Turbo for speed/cost.
        const falModels: FalFluxModelId[] = shouldUseModal
          ...
          : [
              ...
              'fal-ai/flux/dev',
            ]; // Z-Image Turbo + Seedream + Flux Dev when Modal not available
      ...
      this.logger.log(
        `Base images queued: ${providers.join(' + ')} = ${
          allJobIds.length
        } total, jobIds=${allJobIds.join(', ')}`
      );
    ...
    throw new Error(
      'No image generation providers available. Set MODAL_WORKSPACE or FAL_KEY in Infisical /apps/api. ComfyUI pod/RunPod are not supported.'
    );
  }
  ...
  async generateFromCharacterDNA(input: {
    ...
    if (!useModal && !useFal) {
      throw new Error(
        'Neither Modal.com nor FAL available. Set MODAL_WORKSPACE or FAL_KEY in Infisical /apps/api. Character generation requires Modal.com or FAL (ComfyUI/RunPod not supported).'
      );
    }
    ...
    // FAL fallback: text-to-image from character prompt (same job result flow as base images)
    const jobId = this.createExternalJobId('fal:flux-dev:0');
    this.externalResults.set(jobId, {
      status: 'in_progress',
      images: [],
      createdAt: Date.now(),
  ...
  private ensureSafeOutfit(outfit: string): string {
    ...
    if (nsfwOutfits.some((nsfw) => outfitLower.includes(nsfw))) {
      ...
      this.logger.warn(
        `Base image generation: Replaced NSFW outfit "${outfit}" with safe "casual" outfit`
      );
      return 'casual';
    }
  ...
  private wizardConfigToCharacterDNA(
    ...
    return {
      ...
      style:
        identity && appearance
          ? `${identity.archetype} ${appearance.style}`
          : undefined,
    };
  }
  ...
  async getJobResults(jobId: string, userId?: string) {
    ...
    if (jobId.startsWith('fal_job_')) {
      ...
      if (!record) {
        return {
          status: 'failed',
          images: [],
          error: 'External job not found (expired)',
        };
      }
    ...
    if (status.status === 'COMPLETED' && status.output) {
      ...
      try {
        ...
        this.logger.log(
          `Uploaded ${storedImages.length} images to storage for job ${jobId}`
        );
  ...
  private sanitizePromptForStrictModels(
    ...
    if (sanitized !== prompt) {
      this.logger.debug(
        `Sanitized prompt for ${modelId}: removed body part descriptions`
      );
    }
  ...
  private async runModalBaseImageJob(params: {
    ...
    try {
      ...
      this.logger.log(
        `Modal base image uploading jobId=${jobId} model=flux-dev`
      );
      ...
      this.logger.log(
        `Modal base image completed jobId=${jobId} model=flux-dev url=${img.url}`
      );
    } catch (e) {
      const message =
      ...
      this.logger.error(
        `Modal base image failed jobId=${jobId} model=flux-dev: ${message}`,
        stack
      );
      this.externalResults.set(jobId, {
        status: 'failed',
  ...
  private async runFalBaseImageJob(params: {
    ...
    try {
      this.logger.log(
        `Fal base image started jobId=${jobId} model=${modelId} (${modelName})`
      );
      ...
      this.logger.log(
        `Fal base image got response jobId=${jobId} model=${modelId} images=${out.imageUrls.length}`
      );
      ...
      this.logger.log(
        `Fal base image downloading jobId=${jobId} model=${modelId} url=${out.imageUrls[0].substring(
          0,
          50
        )}...`
      );
      ...
      this.logger.log(
        `Fal base image uploading jobId=${jobId} model=${modelId}`
      );
      const { images: stored } = await this.imageStorage.uploadImages(
        [base64],
        {
      ...
      this.logger.log(
        `Fal base image completed jobId=${jobId} model=${modelId} (${modelName}) url=${img.url}`
      );
    } catch (e) {
      ...
      if (isContentPolicyError && isStrictModel) {
        this.logger.warn(
          `Content policy violation for ${modelId}, trying more aggressive sanitization...`
        );
        ...
        try {
          ...
          if (aggressiveSanitized.length > 20) {
            ...
            this.logger.log(
              `Retrying ${modelId} with aggressively sanitized prompt`
            );
            ...
            if (retryOut.imageUrls && retryOut.imageUrls.length > 0) {
              ...
              this.logger.log(
                `Fal base image completed (retry) jobId=${jobId} model=${modelId} (${modelName})`
              );
      ...
      this.logger.error(
        `Fal base image failed jobId=${jobId} model=${modelId} (${modelName}): ${message}`,
        stack
      );
      this.externalResults.set(jobId, {
        status: 'failed',
  ...
  async healthCheck(): Promise<{
    ...
    return {
      available,
      recommendedWorkflow: available
        ? this.comfyuiAdapter.getRecommendedWorkflow()
        : undefined,
    };
  }
}

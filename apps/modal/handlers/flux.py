...
Handles Flux Schnell, Flux Dev, and Flux Dev + LoRA text-to-image generation.
"""
...
class FluxHandler:
    ...
    def _flux_dev_lora_impl(self, item: dict) -> Response:
        ...
        if not ensure_lora_symlink(lora_filename):
            raise HTTPException(
                status_code=404,
                detail=f"LoRA not found: {lora_filename}. Train it first using the LoRA training endpoint."
            )

...
Isolated handler for SeedVR2 app - no dependencies on other handlers.
"""
    ...
    def _seedvr2_impl(self, item: dict) -> Response:
        ...
        if workflow_api_path.exists():
            ...
            try:
                ...
                print(f"⚠️  Converter endpoint not available: {e}. Using direct API execution.")
                # For now, raise error - workflow handler would require importing from old handlers
                raise ValueError(f"Workflow conversion failed: {e}. Please provide API format workflow.")
        ...
        if workflow_api:
            ...
            try:
                ...
                print(f"❌ SeedVR2 API execution failed: {error_msg}")

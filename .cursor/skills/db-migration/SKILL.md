---
name: db-migration
description: Creates and applies database migrations using Drizzle ORM following RYLA schema patterns. Use when modifying database schemas, creating migrations, or when the user mentions database changes, migrations, or schema updates.
---

# Database Migration

Creates and applies database migrations using Drizzle ORM following RYLA schema patterns and best practices.

## Quick Start

When creating a database migration:

1. **Modify Schema** - Update schema files in `libs/data/src/schema/`
2. **Generate Migration** - Run `pnpm db:generate` to create migration files
3. **Review SQL** - Check generated SQL matches your intent
4. **Rename File** - Rename migration file for clarity (optional)
5. **Apply Migration** - Run `pnpm db:migrate` to apply
...
**Follow schema patterns** - See `drizzle-schema-style-guide.mdc` rule for conventions.
...
- **Schema Patterns**: See `drizzle-schema-style-guide.mdc` rule
- **Data Access**: See `data-access.mdc` rule

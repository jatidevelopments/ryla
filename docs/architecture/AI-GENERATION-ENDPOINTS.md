...
```
SFW Content:     Modal.com > Fal.ai (fallback when Modal unavailable)
SFW + Reference: Modal.com > Fal.ai (fal-ai/flux-pulid)
NSFW Content:    Modal.com > ComfyUI Pod
```

### FAL fallback when Modal is unavailable

When `MODAL_WORKSPACE` is not set or Modal is down, the API uses FAL (if `FAL_KEY` is set) for SFW flows:

| Flow                             | FAL fallback                                       | Notes                                                |
| -------------------------------- | -------------------------------------------------- | ---------------------------------------------------- |
| Base image generation            | ✅ fal-ai/flux/dev, fal-ai/z-image/turbo, Seedream | 2 images per model                                   |
| Character DNA generation         | ✅ fal-ai/flux/dev                                 | Single image from character prompt                   |
| Profile picture set (SFW only)   | ✅ fal-ai/z-image/turbo                            | Text-to-image; no face consistency                   |
| Profile picture regenerate (SFW) | ✅ fal-ai/z-image/turbo                            | Single image                                         |
| Studio generation                | ✅ fal-ai/flux/schnell (or chosen modelId)         | Provider auto-switches to FAL when Modal unavailable |

NSFW flows (profile pictures with NSFW, studio NSFW) require Modal or ComfyUI; no FAL fallback.
...
function selectStudioEndpoint(input: StudioInput, provider: Provider): string {
  ...
  // Standard endpoints
  if (provider === 'fal') {
    return input.quality === 'fast' ? 'fal-ai/flux/schnell' : 'fal-ai/flux/dev';
  }
